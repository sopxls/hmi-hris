...
// Save new employee
if (editIndex !== null) {
registration_emp[editIndex] = newEmployee;
editIndex = null;
} else {
registration_emp.push(newEmployee);
}
...
registration_emp.forEach((emp, index) => {
...
function editEmployee(index) {
  ...
  let emp = registration_emp[index];  // Correctly reference the selected employee
...
function deleteEmployee(index) {
  let empID = registration_emp[index].empID; // Get employee ID
...
function loadEmployees() {
  fetch("fetch_registration.php")
      ...
      .then(data => {
          ...
          registration_emp = Array.isArray(data) ? data : []; // Ensure it's an array
          renderTable();
      })
...
if (!registration_emp || !registration_emp[index]) {
    console.error("❌ Error: Employee data not found for index:", index);
    return;
}

let emp = registration_emp[index];
