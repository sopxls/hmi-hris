...
// Save new employee
if (editIndex !== null) {
finance_emp[editIndex] = newEmployee;
editIndex = null;
} else {
finance_emp.push(newEmployee);
}
...
function editEmployee(index) {
  ...
  let emp = finance_emp[index];  // Correctly reference the selected employee
...
function deleteEmployee(index) {
  let empID = finance_emp[index].empID; // Get employee ID

  if (!confirm(`Are you sure you want to delete employee ID ${empID}?`)) {
      return; // Stop if user cancels
  }
  fetch('delete_finance.php', {
      method: 'POST',
...
function loadEmployees() {
  fetch("fetch_finance.php")
      .then(response => response.json())
      .then(data => {
          console.log("🟢 Fetched Employee Data:", data); // Debug fetched data

          finance_emp = Array.isArray(data) ? data : []; // Ensure it's an array
          renderTable();
      })
