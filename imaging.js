...
// Save new employee
if (editIndex !== null) {
imaging_emp[editIndex] = newEmployee;
editIndex = null;
} else {
imaging_emp.push(newEmployee);
}
...
function editEmployee(index) {
  ...
  let emp = imaging_emp[index];  // Correctly reference the selected employee
...
function deleteEmployee(index) {
  let empID = imaging_emp[index].empID; // Get employee ID

  if (!confirm(`Are you sure you want to delete employee ID ${empID}?`)) {
      return; // Stop if user cancels
  }
  fetch('delete_imaging.php', {
      method: 'POST',
...
function loadEmployees() {
  fetch("fetch_imaging.php")
      .then(response => response.json())
      .then(data => {
          console.log("🟢 Fetched Employee Data:", data); // Debug fetched data

          imaging_emp = Array.isArray(data) ? data : []; // Ensure it's an array
          renderTable();
      })
