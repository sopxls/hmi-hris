...
// Save new employee
if (editIndex !== null) {
medical_emp[editIndex] = newEmployee;
editIndex = null;
} else {
medical_emp.push(newEmployee);
}
...
function editEmployee(index) {
  ...
  let emp = medical_emp[index];  // Correctly reference the selected employee
...
function deleteEmployee(index) {
  let empID = medical_emp[index].empID; // Get employee ID
...
function loadEmployees() {
  fetch("fetch_medical.php")
      .then(response => response.json())
      .then(data => {
          console.log("🟢 Fetched Employee Data:", data); // Debug fetched data

          medical_emp = Array.isArray(data) ? data : []; // Ensure it's an array
          renderTable();
      })
