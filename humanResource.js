...
// Save new employee
if (editIndex !== null) {
hr_emp[editIndex] = newEmployee;
editIndex = null;
} else {
hr_emp.push(newEmployee);
}
...
function editEmployee(index) {
  ...
  let emp = hr_emp[index];  // Correctly reference the selected employee
...
function deleteEmployee(index) {
  let empID = hr_emp[index].empID; // Get employee ID

  if (!confirm(`Are you sure you want to delete employee ID ${empID}?`)) {
      return; // Stop if user cancels
  }
  fetch('delete_hremp.php', {
      method: 'POST',
...
function loadEmployees() {
  fetch("fetch_hremp.php")
      .then(response => response.json())
      .then(data => {
          console.log("🟢 Fetched Employee Data:", data); // Debug fetched data

          hr_emp = Array.isArray(data) ? data : []; // Ensure it's an array
          renderTable();
      })
