...
.addEventListener("click", function (event) {
  ...
  let employeeData = laboratory.map((emp) => {
...
// Save new employee
if (editIndex !== null) {
laboratory[editIndex] = newEmployee;
editIndex = null;
} else {
laboratory.push(newEmployee);
}
...
function editEmployee(index) {
  ...
  let emp = laboratory[index];  // Correctly reference the selected employee
...
function deleteEmployee(index) {
  let empID = laboratory[index].empID; // Get employee ID

  if (!confirm(`Are you sure you want to delete employee ID ${empID}?`)) {
      return; // Stop if user cancels
  }
  fetch('delete_laboratory.php', {
      method: 'POST',
...
function loadEmployees() {
  fetch("fetch_laboratory.php")
      .then(response => response.json())
      .then(data => {
          console.log("🟢 Fetched Employee Data:", data); // Debug fetched data

          laboratory = Array.isArray(data) ? data : []; // Ensure it's an array
          renderTable();
      })
