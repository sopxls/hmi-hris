...
// Save new employee
if (editIndex !== null) {
customer_emp[editIndex] = newEmployee;
editIndex = null;
} else {
customer_emp.push(newEmployee);
}
...
customer_emp.forEach((emp, index) => {
...
function editEmployee(index) {
  ...
  let emp = customer_emp[index];  // Correctly reference the selected employee
...
function deleteEmployee(index) {
  let empID = customer_emp[index].empID; // Get employee ID
...
function loadEmployees() {
  fetch("fetch_customer.php")
      ...
      .then(data => {
          ...
          customer_emp = Array.isArray(data) ? data : []; // Ensure it's an array
          renderTable();
      })
...
if (!customer_emp || !customer_emp[index]) {
    console.error("❌ Error: Employee data not found for index:", index);
    return;
}

let emp = customer_emp[index];
