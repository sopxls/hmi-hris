...
// Save new employee
if (editIndex !== null) {
psychometry_emp[editIndex] = newEmployee;
editIndex = null;
} else {
psychometry_emp.push(newEmployee);
}
...
psychometry_emp.forEach((emp, index) => {
...
function editEmployee(index) {
  ...
  let emp = psychometry_emp[index];  // Correctly reference the selected employee
...
function deleteEmployee(index) {
  let empID = psychometry_emp[index].empID; // Get employee ID

  if (!confirm(`Are you sure you want to delete employee ID ${empID}?`)) {
      return; // Stop if user cancels
  }
  fetch('delete_psychometry.php', {
      method: 'POST',
...
function loadEmployees() {
  fetch("fetch_psychometry.php")
      .then(response => response.json())
      .then(data => {
          console.log("🟢 Fetched Employee Data:", data); // Debug fetched data

          psychometry_emp = Array.isArray(data) ? data : []; // Ensure it's an array
          renderTable();
      })
...
if (!psychometry_emp || !psychometry_emp[index]) {
    console.error("❌ Error: Employee data not found for index:", index);
    return;
}

let emp = psychometry_emp[index];
