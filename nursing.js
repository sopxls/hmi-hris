...
// Save new employee
if (editIndex !== null) {
nursing_emp[editIndex] = newEmployee;
editIndex = null;
} else {
nursing_emp.push(newEmployee);
}
...
function editEmployee(index) {
  ...
  let emp = nursing_emp[index];  // Correctly reference the selected employee
...
function deleteEmployee(index) {
  let empID = nursing_emp[index].empID; // Get employee ID

  if (!confirm(`Are you sure you want to delete employee ID ${empID}?`)) {
      return; // Stop if user cancels
  }
  fetch('delete_nursing.php', {
      method: 'POST',
...
function loadEmployees() {
  fetch("fetch_nursing.php")
      .then(response => response.json())
      .then(data => {
          console.log("🟢 Fetched Employee Data:", data); // Debug fetched data

          nursing_emp = Array.isArray(data) ? data : []; // Ensure it's an array
          renderTable();
      })
