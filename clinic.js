...
// Save new employee
if (editIndex !== null) {
clinic_emp[editIndex] = newEmployee;
editIndex = null;
} else {
clinic_emp.push(newEmployee);
}
...
function editEmployee(index) {
  ...
  let emp = clinic_emp[index];  // Correctly reference the selected employee
...
function deleteEmployee(index) {
  let empID = clinic_emp[index].empID; // Get employee ID

  if (!confirm(`Are you sure you want to delete employee ID ${empID}?`)) {
      return; // Stop if user cancels
  }
  fetch('delete_clinic.php', {
      method: 'POST',
...
function loadEmployees() {
  fetch("fetch_clinic.php")
      .then(response => response.json())
      .then(data => {
          console.log("🟢 Fetched Employee Data:", data); // Debug fetched data

          clinic_emp = Array.isArray(data) ? data : []; // Ensure it's an array
          renderTable();
      })
